...
const TIER_POOLS: Record<string, string[]> = {
  ULTRA: ["Oc-uni/gpt-6-astra", "Atria-Dawn-Preview/Atria-Dawn-Preview", "Oc-full/cc/claude-opus-5", "Oc-full/xai/grok-4.6", "Oc-full/qwen/qwen3.8-max"],
...
// Preset Combo Terisolasi Bebas Tumpang Tindih
const COMBO_PRESETS: Record<string, { name: string; engines: string[]; arbiter: string }> = {
  ...
  ultra: {
    ...
    engines: ["Oc-full/xai/grok-4.6", "Oc-full/cc/claude-opus-5", "Oc-full/qwen/qwen3.8-max"],
    arbiter: "Oc-full/cc/claude-opus-5"
  },
  high: {
    name: "High Logic & Code",
    engines: ["cx/gpt-5.6-terra", "ag/gemini-3.8-flash-high", "Oc-uni/deepseek/deepseek-v4-pro"],
        ...
        if (isCombo) {
          ...
          await Promise.all(cfg.engines.map(async (eng) => {
            try {
              ...
              perspectives[eng] = `(Engine timeout/antrean: ${e.message})`;
            }
          }));

          const judgePrompt = `Anda Arbiter Konsensus (${cfg.name}).
Pertanyaan: ${messages[messages.length - 1]?.content}
Berikut perspektif dari ketiga engine:
${cfg.engines.map(e => `[${e}]:${perspectives[e]}`).join("\n\n")}
Susun konsensus jawaban yang berbobot, akurat, dan terstruktur.`;

          let finalReply = "";
          try {
            finalReply = await fetchFromUpstream(cfg.arbiter, [{ role: "user", content: judgePrompt }]);
          } catch (_) {
            const valid = Object.values(perspectives).filter(t => !t.startsWith("(Engine timeout"));
            finalReply = valid[0] || Object.values(perspectives)[0] || "Gagal menyusun konsensus.";
          }
